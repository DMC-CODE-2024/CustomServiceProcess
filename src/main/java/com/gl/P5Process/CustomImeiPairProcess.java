...
public class CustomImeiPairProcess {
    ...
    public static void startService(Connection conn, String table, String lastRunTime, String remark, String source,
            String status) {
        // gdce_data
        var subQuery = " select a.imei  from " + table + " a, imei_pair_detail b " +
                " where a.imei=b.imei and a.created_on >= ( select IFNULL(value, '2000-01-01') from sys_param where tag ='"
                + lastRunTime + "' ) ";
        ...
        deleteFromImeiPair(conn,
                " select imei from " + table
                        + " where  created_on >= ( select IFNULL(value, '2000-01-01') from sys_param where tag ='"
                        + lastRunTime + "' )  ");
    ...
    public static void updateNwl(Connection conn, String table, String lastRunTime, String status) {
        var q = "update app.national_whitelist set gdce_imei_status = " + status
                + " , gdce_modified_time =CURRENT_TIMESTAMP" +
                "  where gdce_imei_status in (0,3)   and imei in( select imei from app." + table
                + " where created_on >= ( select IFNULL(value, '2000-01-01') from sys_param where tag ='" + lastRunTime
                + "' )  )  ";
        runQuery(conn, q);
    }
    ...
    private static void insertNwlFromGdceOnPairRecordTime(Connection conn, String lastRunTime, String status) {
        String a = "insert into  app.national_whitelist(action,actual_imei,actual_operator,created_filename,created_on_date,failed_rule_date, "
                ...
                +
                " and  imei_pair_detail.record_time is not null and gdce_data.created_on >= ( select IFNULL(value, '2000-01-01') from sys_param where tag ='"
                + lastRunTime + "' )  ))   ";
        runQuery(conn, a);
    }
    ...
    private static void insertInExceptionListHis(Connection conn, String subquery, String remark) { // GDCE_TAX_PAID
        String q = "insert into exception_list_his (actual_imei, imei,imsi , msisdn ,operator_id , operator_name, complaint_type, expiry_date , mode_type , request_type , txn_id , user_id , user_type ,tac ,remark,source ,action ,action_remark ,operation )"
                +
                " select actual_imei, imei,imsi , msisdn ,operator_id , operator_name, complaint_type, expiry_date , mode_type , request_type , txn_id , user_id , user_type ,tac ,remark,'"
                + remark + "' ,'DELETE','" + remark + "' , 0 from exception_list " +
    ...
    private static void insertInImeiPairHis(Connection conn, String subquery, String remark) {
        String q = "insert into imei_pair_detail_his ( allowed_days,imei ,imsi,msisdn,pairing_date ,record_time,file_name,gsma_status,pair_mode, operator,expiry_date , action,action_remark) "
                +
                " select allowed_days,imei ,imsi,msisdn, pairing_date ,record_time, file_name,gsma_status, pair_mode, operator,expiry_date ,'DELETE','"
                + remark + "' from  imei_pair_detail " +
                "where imei in( " + subquery + " )";
        runQuery(conn, q);
    }
    ...
    private static void removeAutoFromBlackList(Connection conn, String subquery, String source) {
        String q = "UPDATE black_list SET source = TRIM(BOTH ',' FROM REPLACE(CONCAT(',', source, ','), '," + source
                + ",', ','))WHERE source LIKE '%" + source + "%'  and  imei in(" + subquery + ")  ";
        runQuery(conn, q);
    }
    ...
    private static void insertInBlackListHis(Connection conn, String subquery, String remark, String source) {
        String q = "insert into black_list_his (actual_imei, imei,imsi , msisdn ,operator_id , operator_name, complaint_type, expiry_date , mode_type , request_type , txn_id , user_id , user_type ,tac ,remark,source ,action ,action_remark ) "
                +
                " select actual_imei, imei,imsi , msisdn ,operator_id , operator_name, complaint_type, expiry_date , mode_type , request_type , txn_id , user_id , user_type ,tac ,remark, source ,'DELETE','"
                + remark + "' from black_list" +
